...
![基金持仓](figures/funds_component.png)

## 项目简介

这是一个基于 **FastAPI + Vue.js + PostgreSQL** 的中国基金实时净收益查询系统，使用 **efinance 和 Tushare** 作为数据源。
...
- 🐛 **修复饼图显示问题**：
  - 解决 Vue DOM 渲染时序导致的饼图不显示问题
  - 在 `fetchStockPositions` 中添加 `await nextTick()` 确保 DOM 完成渲染
  - 饼图现在能正确显示 Top 10 重仓股的持仓占比
  - 添加详细的调试日志，便于问题排查
- 🔧 **前端优化**：
  - 改进 Vue 组件生命周期管理
  - 确保 DOM 元素在初始化图表前已完全渲染
  - 优化 watchEffect 清理逻辑，避免图表实例被意外销毁

### 最新功能 (v1.7.4)

- 📊 **实时股票行情功能**：
  - 恢复基于股票持仓的实时估值功能
  - 使用 efinance + Tushare 双数据源，自动降级
  - 交易时间每 1 分钟自动更新，避免频繁访问 API
  - 非交易时间显示友好提示
- 🔧 **编码问题处理**：
  - 新增编码转换工具模块 `encoding.py`
  - 自动检测和转换 GBK/UTF-8 编码
  - 修复股票名称乱码问题
  - 数据库连接显式指定 UTF-8 编码
- 📈 **持仓详情展示**：
  - 持仓表格：股票代码、名称、持仓股数、市值、占净值比例、报告期
  - 持仓占比饼图：Top 10 重仓股的可视化展示
  - 持仓数据质量检查 API
- 📚 **文档更新**：
  - README 新增实时行情功能说明
  - 添加编码处理机制文档
...
项目统一使用 UTF-8 编码，但考虑到 Windows 系统 GBK 编码和部分数据源的编码问题，实现了自动编码转换：

- **编码检测**：使用 `chardet` 库自动检测数据编码
- **编码转换**：GBK/GB2312/GB18030 → UTF-8
- **乱码修复**：自动修复常见的 Mojibake（字符编码错误）
- **名称验证**：验证中文名称是否有效，检测乱码
...
- **持仓表格**：股票代码、名称、持仓股数、市值、占净值比例、报告期
- **持仓占比饼图**：Top 10 重仓股的可视化展示
- **实时估值**：基于持仓股票实时价格计算基金实时估值
- **自动刷新**：交易时间内每 1 分钟自动更新数据
...
1. **交易时间限制**
   - 实时行情仅在交易时间可用（工作日 9:30-15:00）
   - 非交易时间显示最新正式净值数据

2. **API 限流**
   - Tushare Pro：每分钟调用次数受限（根据积分等级）
   - 建议设置合理的刷新频率（默认 1 分钟）

3. **数据延迟**
   - 实时行情通常有 3-15 秒延迟
   - 持仓数据来自基金季报/年报，有较长时间滞后

4. **编码问题**
   - 如遇股票名称乱码，可能是编码转换失败
   - 查看 `backend/app/utils/encoding.py` 了解编码处理逻辑
   - 检查系统编码设置（Windows 控制面板 → 区域 → 管理 → 更改系统区域设置）
...
**Bug 修复（关键）**：
- 🐛 **修复饼图 DOM 渲染时序问题**
  - 问题原因：`v-if` 导致 DOM 异步渲染，但 `initPieChart()` 在渲染前被调用
  - 解决方案：在 `fetchStockPositions` 中添加 `await nextTick()` 等待 DOM 渲染完成
  - 影响：饼图现在能正确显示 Top 10 重仓股的持仓占比
- 🔧 **Vue 生命周期优化**：
  - 改进组件渲染时序管理
  - 确保模板引用（ref）在访问前已完成 DOM 更新
  - 添加详细的调试日志（🥧 前缀）便于问题排查
- 📊 **数据处理增强**：
  - 支持权重数据（weight）和市值数据（market_value）双数据源
  - 当权重数据为空时，自动使用市值计算占比作为 fallback
  - 修复字符串转数字的类型转换问题
- ✅ 彻底解决基金详情页饼图不显示的问题
...
**重大更新 - 数据源替换**：
- 🔄 **新增东方财富网数据源**：使用爬虫技术获取基金持仓，完全免费无限制
- ⚡ **优先级策略**：东方财富网（优先） → Tushare API（降级）
- 🚫 **移除 Tushare 依赖**：不再需要 Tushare Token 即可同步持仓
- 📊 **数据源标识**：`sina_eastmoney` 表示来自东方财富网
